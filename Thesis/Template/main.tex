...
is added to the train set and the process repeated. \citep{sima2018timeseries}. To test the generalization performance, however, no
rolling window approach is applied on the test set. Instead, the model is supposed to predict the entire test set based on the parameters
...
included in the model (p), the number of times the data needs to be differenced (d) and the size of the moving average window (q) \citep{sima2018timeseries}.\\
...
In addition to only the price as an input feature, trading volume and sentiment were also included as scaled input features.
...
If the visualization contains multiple line charts, the best result is drawn as a solid line and the rest as a dashed line.
...
\begin{figure}[!htb]
    \centering
    \includegraphics[scale=0.45]{NB_hyperparam.png}
...
After three epochs the model seems to start overfitting a little bit. This can be concluded, as the valuation set accuracy starts dropping, while the training set
...
\begin{figure}[!htb]
    \centering
    \includegraphics[scale=0.5]{LSTM_Accuracy_Val_Set.PNG}
...
\begin{figure}[!htb]
    \centering
    \includegraphics[scale=0.5]{BERT_Accuracy_Validation_Set.PNG}
...
Based on that the accuracy BERT was chosen to label the rest of the data. To compare how well the models generalize, the evaluation metrics
of the testsets are outlined in Table \ref{tab:results}.
The commonly used VADER lexicon is included for illustrative purposes.
As can be seen VADER underperforms the NB baseline model by a big margin.
Both BERT and the LSTM, however, show quite strong results even though they overfit quite a bit.
...
learning its optimal weights yet and more epochs may be benefitial. The RMSE and Loss of the training and validation set can be found in \nameref{appendix:lstm_models_rmse_loss}.
...
\begin{figure}
    \centering
    \includegraphics[scale = 0.55]{Best_Params_Of_Model_RMSE_Validation_Set.png}
...
\begin{figure}
    \centering
    \includegraphics[scale = 0.55]{Only_Price_Params_RMSE_Validation_Set.png}
...
As the plot of the predition of the test set in Figure \ref{fig:price_forecast_best_model}, it can be seen that the model does a relatively good job at generalizing.
Even though it does not overlap with the actual stock price at most occasions, it generally got the direction correct.
...
As the 'to the moon' WallStreetBets movement had a tremendous impact on the financial markets and the lives of individuals, the methodology proposed in this thesis aims to accurately measure and monitor the subreddit. As a result, the disruptions to the financial markets may be avoided and stability be ensured.
The underlying hypothesis of this thesis is that by accurately measuring the sentiment of the forum, changes in the stock price can be predicted. This is not with its own challenges, however. First of all, the terminology used on the forum is currently not covered by any sentiment lexicas. Additionally, there are no datasets that contain labeled data of the forum. As a result, this thesis proposes an Active Learner to annotate the data for a fraction of the total cost. However, the performance of the Active Learner was not as high as hoped. This can be attributed to the ambiguity of posts at the document level. Even when manually labeling the data it was not always entirely clear on how to classify a post. While the advantages of an Active Learner still seem promising, a different labeling approach, such as part-of-speech tagging might be a better solution than on the document level.
The data obtained by the Active Learner can subsequently be used as the ground truth data which can be fed into various sentiment analysis methods.
Even though BERT achieved the best performance, generalization, etc.
Once the sentiment is obtained, it can be tested whether it actually contributes to the predictive capabilities. Even though predictions are quite accurate when including sentiment, especially generalization -> how does it do?
It is shown that a LSTM model that only makes predictions based on price, actually performs better than a model that includes sentiment. One possible explanation for that is the efficient market hypothesis, which assumes that all available information is already priced into the stock price. Find paper. Furthermore, the results show that an LSTM performs better than ARIMA.
Therefore, it can be concluded that while the inclusion of sentiment shows better predictive capabilities than for example the feature trading volume, it does not enhance the performance of a model that only relies on price.
I strongly believe that this thesis contributes to the existing literature on WallStreetBets.
...
\subsection{Appendix LSTM\_Sentiment\_Classifier}
\label{appendix:LSTM_Sentiment_Classifier}
\begin{figure}
    \centering
    \includegraphics[scale=0.5]{LSTM_Accuracy.png}
...
\pagebreak
\subsection{Appendix BERT\_Sentiment\_Classifier}
\label{appendix:BERT_Sentiment_Classifier}
\begin{figure}[!htb]
    \centering
    \includegraphics[scale=0.5]{BERT_Accuracy.png}
...
\subsection{Appendix RMSE and Loss of Input Features}
...
\subsection{Appendix RMSE and Loss of Best Model}
