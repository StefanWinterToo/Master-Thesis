...
\begin{itemize}
    \item[RQ3] \emph{Do machine learning models show stronger predictive capabilities for changes in the stock price of Gamestop if sentiment is included as a feature, based on the predefined evaluation MAE and RMSE?}
\end{itemize}
...
% Write why stock price prediction is a challenging task.
...
time correlation between information. Even though the results seem promising whenusing an LSTM to predict the stock price of Apple, the performance of the LSTM 
was enhanced by accounting for sentiment as well \citep{jin2020lstmsentiment}.
...
ability to catch time-series specific features \citep{vuong2021forecasting}.

While there are various studies of applying the aforementioned stock-price forecasting methods, almost none do so for the WallStreetBets subreddit. 
