...
Explain challenges of stock price prediction (i.e. train test split) \\

\noindent\textbf{ARIMA}\\
% https://stats.stackexchange.com/questions/394796/should-my-time-series-be-stationary-to-use-arima-model
Since ARIMA requires some prerequisites to create good results, those need to be accounted for before applying the model.
First of all, ARIMA does not work with seasonal data. Seasonality in the time series can be tested by applying \emph{XYZ}.
...
As a result, the applied dickey fuller test can be used to test for stationarity. If the null hypothesis can be rejected, the time series is stationary at a significance level.
For the case at hand, the p-value is almost 0, which indicates statistical significane at the 1\% level.
