...
highest accuracy of 80.4\% can be achieved with by setting hyperparameters alpha to one and fit\_prior to False. Since this baseline estimator
performs much better, than just randomly guessing the majority class (bullish), this result is quite good. \\

\begin{figure}[!htb]
    \centering
    \includegraphics[width=\textwidth]{NB_hyperparam.png}
...
This can be concluded, as the valuation set accuracy in Figure \ref{fig:lstm_acc} starts dropping, while the training set
accuracy keeps improving. The same can be concluded for the loss as visualized in Figure \ref{fig:lstm_loss}. \\
...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=\textwidth]{LSTM_Accuracy.png}
    \caption{Accuracy of LSTM with varying hyperparameters}
    \label{fig:lstm_acc}
\end{figure}
...
\begin{figure}[!htb]
    ...
    \caption{Loss of LSTM with varying hyperparameters}
    \label{fig:lstm_loss}
\end{figure}
...
\begin{table}[!htb]
    \caption{Test scores for NB, LSTM and BERT - Needs to be updated!}
    \label{tab:results}
    \centering
    \small
