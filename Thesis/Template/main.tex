...
\begin{abstract}
    ...
    to measure the sentiment of WallStreetBets. % Write a little bit more about stock prediction, once I have the results.
\end{abstract}
...
\begin{figure} % Also need a legend
    \centering
...
Of all obtained posts, 10\% or 17,955 were manually labeled as either bearish, neutral or bullish. This was done by using a graphical user interface that displayed the title and
text of every tenth Reddit post. Of all manually labelled data 3479 (19.4\%) are bearish, 5119 (28.5\%) are neutral and 9357 (52.1\%) are bullish. \\
...
by plotting the stock chart and other metrics. See \emph{appendix B} for more. %Still need to write about that.
...
lastly the stock price prediction models.

\subsection{Active Learner}
The Active Learner is represented as  a line chart with the accuracy on the y-axis and the respective query instance on the y-axis.
As can be seen in Figure 2, the performance of the Active Learner hit a plateau relatively quickly. It is hypothesized that
this plateau was reached for two reasons. First, by labelling the data at the document level, a document can contain both bearish
and bullish statements. Those contradictions not only make it hard for the model to learn the structure of the input, but also for 
the human annotator to label the data properly. Additionally, by having two different human oracles to label the data, inconsistencies
may occur.
...
rest of the dataset. The best performing hyperparameter-set of the NB classifier achieved accuracy of 0.8, the LSTM XYZ and BERT XYZ.
...
of the testsets are outlined in table XYZ. As can be seen the commonly used VADER lexicon underperforms the NB baseline model by a big margin.
...
\subsection{Stock Price Prediction}
The best performing stock price prediction model was selected based on the RMSE, where the LSTM model that includes sentiment
outperformed the other models. Based on the validation set, the ARIMA model scored XYZ, the LSTM without sentiment achieved
XYZ and the LSTM that includes other features XYZ.

The generalization performance of the test set is represented in table ABC.
\begin{table}
    \caption{Test scores for ARIMA and LSTM}
    ...
    \begin{tabular}{lllrr}
        \toprule
            ...
            \midrule
            \multirow{1}{*}{LSTM without Sentiment}       & 9.1          & 82.8          & 7.3 \\
            \midrule
            \multirow{1}{*}{BERT with Sentiment}       & \textbf{7.8}          & \textbf{60.8}          & \textbf{5.1} \\
            \midrule
            \multirow{1}{*}{nothing}      & 0          & 0          & 0 \\
...
To better understand the results in table ABC, the price predictions were plotted over the actual stock data as red dashed lines.
Both the actual and predicted stock price is on the y-axis plotted over time.
The predictions start exactly at the position of the test set.

As can be seen in figure ABC, ARIMA seems to simply remember the past longer term trend and extrapolates it out into the future. This
suggests that the model overfitted the data and does not perform well.

\begin{figure}[!htb]
    \centering
    \includegraphics[scale=0.4]{prediction_arima.png}
    \caption{ARIMA}
\end{figure}

The LSTM without sentiment features, on the other hand seems to perform much better than the baseline ARIMA model. As can be seen in
figure ABC, the model is able to capture the general trend of the stock price, even though there seems to be some minor divergence.

\begin{figure}[!htb]
    \centering
    \includegraphics[scale=0.4]{prediction_lstm_no_sentiment.png}
    \caption{LSTM without Sentiment}
\end{figure}

The LSTM that includes sentiment features, however, fits the data almost perfectly. 

\begin{figure}[!htb]
    \centering
    \includegraphics[scale=0.4]{prediction_lstm_with_sentiment.png}
    \caption{LSTM with Sentiment}
\end{figure}
